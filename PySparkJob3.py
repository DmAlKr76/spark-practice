...
def process(spark, flights_path, result_path):
    ...
    df = data.groupBy(F.col('ORIGIN_AIRPORT')) \
        .agg(F.round(F.avg(F.col('DEPARTURE_DELAY')), 6).alias('avg_delay'),
             F.min(F.col('DEPARTURE_DELAY')).alias('min_delay'),
             F.max(F.col('DEPARTURE_DELAY')).alias('max_delay'),
             F.round(F.corr(F.col('DEPARTURE_DELAY'), F.col('DAY_OF_WEEK')), 6).alias("corr_delay2day_of_week")) \
